...
int main() {
  ...
  std::cout << " -> Resetting System Audio (Cleaning old settings)..."
            << std::endl;
  ...
  if (!g_eq.Initialize()) {
    std::cout << " [Error] Failed to initialize EQ Controller (Shared Memory)."
              << std::endl;
    return 1;
  }

  std::cout << " -> Restarting Audio Services to apply native changes..."
            << std::endl;
