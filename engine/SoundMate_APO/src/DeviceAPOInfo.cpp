...
#define apoRegistrationKeyPath                                                 \
  L"HKEY_CLASSES_ROOT\\AudioEngine\\AudioProcessingObjects"
#define clsidKeyPath L"HKEY_CLASSES_ROOT\\CLSID"
#define commonKeyPath                                                          \
  L"HKEY_LOCAL_"                                                               \
  L"MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\MMDevices\\Audio"
...
static const wchar_t *allowSilentBufferValueName =
    L"AllowSilentBufferModification";
static const wchar_t *disableAutoAdjustValueName =
    L"DisableAutomaticAdjustment";
static const wchar_t *versionValueName = L"Version";
static const wchar_t *connectionValueName =
    L"{a45c254e-df1c-4efd-8020-67d146a850e0},2";
static const wchar_t *deviceValueName =
    L"{b3f8fa53-0004-438e-9003-51a46e139bfc},6";
static const wchar_t *combinedDeviceValueName =
    L"{b3f8fa53-0004-438e-9003-51a46e139bfc},41";
static const wchar_t *formatValueName =
    L"{f19f064d-082c-4e27-bc73-6882a1bb8e4c},0";
static const wchar_t *channelMaskValueName =
    L"{1da5d803-d492-4edd-8c23-e0c0ffee7f0e},3";
static const wchar_t *lfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},1";
static const wchar_t *gfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},2";
static const wchar_t *sfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},5";
static const wchar_t *mfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},6";
static const wchar_t *efxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},7";
static const wchar_t *multiSfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},13";
static const wchar_t *multiMfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},14";
static const wchar_t *multiEfxGuidValueName =
    L"{d04e05a6-594b-4fb6-a80d-01af5eed7d1d},15";
static const unsigned allGuidValueNameCount = 5;
static const wchar_t *allGuidValueNames[] = {lfxGuidValueName, gfxGuidValueName,
                                             sfxGuidValueName, mfxGuidValueName,
                                             efxGuidValueName};
...
static const wchar_t *fxTitleValueName =
    L"{b725f130-47ef-101a-a5f1-02608c9eebac},10";
static const wchar_t *sfxProcessingModesValueName =
    L"{d3993a3f-99c2-4402-b5ec-a92a0367664b},5";
static const wchar_t *mfxProcessingModesValueName =
    L"{d3993a3f-99c2-4402-b5ec-a92a0367664b},6";
static const wchar_t *efxProcessingModesValueName =
    L"{d3993a3f-99c2-4402-b5ec-a92a0367664b},7";
static const wchar_t *defaultProcessingModeValue =
    L"{C18E2F7E-933D-4965-B7D1-1EEF228D2AF3}";
static const wchar_t *disableEnhancementsValueName =
    L"{1da5d803-d492-4edd-8c23-e0c0ffee7f0e},5";
static const wchar_t *installVersion = L"2";
static PROPERTYKEY guidPropertyKey = {{0x1da5d803, 0xd492, 0x4edd, 0x8c, 0x23,
...
vector<shared_ptr<AbstractAPOInfo>> DeviceAPOInfo::loadAllInfos(bool input) {
  ...
  vector<wstring> deviceGuidStrings =
      RegistryHelper::enumSubKeys(input ? captureKeyPath : renderKeyPath);
  wstring defaultDeviceGuid = getDefaultDevice(input);
  for (vector<wstring>::iterator it = deviceGuidStrings.begin();
       it != deviceGuidStrings.end(); it++) {
...
wstring DeviceAPOInfo::getDefaultDevice(bool input, int role) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = enumerator->GetDefaultAudioEndpoint(input ? eCapture : eRender,
                                             (ERole)role, &endPoint);
    if (SUCCEEDED(hr)) {
...
bool DeviceAPOInfo::checkProtectedAudioDG(bool fix) {
  ...
  if (!RegistryHelper::valueExists(protectedDGKeyPath, protectedDGValueName) ||
      RegistryHelper::readDWORDValue(protectedDGKeyPath,
                                     protectedDGValueName) != 1) {
    result = false;

    if (fix)
      RegistryHelper::writeDWORDValue(protectedDGKeyPath, protectedDGValueName,
                                      1);
  }
...
bool DeviceAPOInfo::load(const wstring &deviceGuid, wstring defaultDeviceGuid) {
  ...
  unsigned long deviceState =
      RegistryHelper::readDWORDValue(keyPath, L"DeviceState");
  if (deviceState & DEVICE_STATE_NOTPRESENT)
    return false;
  // somehow, disabled devices do not actually use DEVICE_STATE_DISABLED in
  // registry but 0x10000000
  ...
  connectionName =
      RegistryHelper::readValue(keyPath + L"\\Properties", connectionValueName);
  deviceName =
      RegistryHelper::readValue(keyPath + L"\\Properties", deviceValueName);

  channelCount = 0;
  sampleRate = 0;
  channelMask = 0;
  if (RegistryHelper::valueExists(keyPath + L"\\Properties", formatValueName)) {
    std::vector<unsigned char> format = RegistryHelper::readBinaryValue(
        keyPath + L"\\Properties", formatValueName);
  ...
  if (channelMask == 0 && RegistryHelper::valueExists(keyPath + L"\\Properties",
                                                      channelMaskValueName))
    channelMask = RegistryHelper::readDWORDValue(keyPath + L"\\Properties",
                                                 channelMaskValueName);
  ...
  if (RegistryHelper::keyExists(keyPath + L"\\FxProperties") &&
      RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                  disableEnhancementsValueName))
    enhancementsDisabled =
        RegistryHelper::readDWORDValue(keyPath + L"\\FxProperties",
                                       disableEnhancementsValueName) != 0;
  ...
  if (!RegistryHelper::keyExists(keyPath + L"\\FxProperties")) {
    ...
    for (int i = 0; i < allGuidValueNameCount; i++) {
      ...
      if (RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                      allGuidValueNames[i])) {
        wstring apoGuidString = RegistryHelper::readValue(
            keyPath + L"\\FxProperties", allGuidValueNames[i]);
    ...
    if (found) {
      ...
      if (RegistryHelper::keyExists(childApoPath L"\\" + deviceGuid)) {
        if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
                                        versionValueName)) {
          version = RegistryHelper::readValue(childApoPath L"\\" + deviceGuid,
                                              versionValueName);
        ...
        if (version == installVersion) {
          if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
                                          preMixChildGuidValueName))
            preMixChildGuid = RegistryHelper::readValue(
                childApoPath L"\\" + deviceGuid, preMixChildGuidValueName);
          if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
                                          postMixChildGuidValueName))
            postMixChildGuid = RegistryHelper::readValue(
                childApoPath L"\\" + deviceGuid, postMixChildGuidValueName);

          currentInstallState.installPreMix =
              foundAt[LFX_INDEX] || foundAt[SFX_INDEX];
          currentInstallState.installPostMix =
              foundAt[GFX_INDEX] || foundAt[MFX_INDEX] || foundAt[EFX_INDEX];
          ...
          if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
            ...
            currentInstallState.allowSilentBufferModification =
                RegistryHelper::readValue(childApoPath L"\\" + deviceGuid,
                                          allowSilentBufferValueName) !=
                L"false";
          if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
                                          disableAutoAdjustValueName))
            currentInstallState.autoAdjust =
                RegistryHelper::readValue(childApoPath L"\\" + deviceGuid,
                                          disableAutoAdjustValueName) ==
                L"false";
        } else {
      ...
      } else if (RegistryHelper::keyExists(childApoPath) &&
                 RegistryHelper::valueExists(childApoPath, deviceGuid)) {
        for (int i = 0; i < allGuidValueNameCount; i++) {
          if (foundAt[i]) {
            originalApoGuids[i] =
                RegistryHelper::readValue(childApoPath, deviceGuid);
      ...
      if (RegistryHelper::isWindowsVersionAtLeast(6, 3)) // Windows 8.1
      {
        ...
        if (RegistryHelper::keyExists(keyPath + L"\\FxProperties") &&
            ...
            !RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                         efxGuidValueName) &&
            !RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                         multiSfxGuidValueName) &&
            !RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                         multiMfxGuidValueName) &&
            !RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                         multiEfxGuidValueName))
          currentInstallState.installMode = INSTALL_LFX_GFX;
        // bluetooth devices may be combined in Windows 11, EFX will not work
        // then
        else if (RegistryHelper::valueExists(keyPath + L"\\Properties",
                                             combinedDeviceValueName))
          currentInstallState.installMode = INSTALL_SFX_MFX;
        else
          currentInstallState.installMode = INSTALL_SFX_EFX;
      }
...
wstring DeviceAPOInfo::getOriginalAPOPreMix() {
  ...
  case INSTALL_LFX_GFX:
    ...
    if (RegistryHelper::isWindowsVersionAtLeast(6, 3)) // Windows 8.1
    {
      if (originalApoGuids[LFX_INDEX] == APOGUID_NOVALUE &&
          originalApoGuids[GFX_INDEX] == APOGUID_NOVALUE)
        guid = originalApoGuids[SFX_INDEX];
    }
    break;
  case INSTALL_SFX_MFX:
    guid = originalApoGuids[SFX_INDEX];
    if (originalApoGuids[SFX_INDEX] == APOGUID_NOVALUE &&
        originalApoGuids[MFX_INDEX] == APOGUID_NOVALUE)
      guid = originalApoGuids[LFX_INDEX];
    break;
  case INSTALL_SFX_EFX:
    guid = originalApoGuids[SFX_INDEX];
    if (originalApoGuids[SFX_INDEX] == APOGUID_NOVALUE &&
        originalApoGuids[EFX_INDEX] == APOGUID_NOVALUE)
      guid = originalApoGuids[LFX_INDEX];
    break;
  }
...
void DeviceAPOInfo::install() {
  if (!selectedInstallState.installPreMix &&
      !selectedInstallState.installPostMix)
    return;
  ...
  if (!RegistryHelper::keyExists(keyPath + L"\\FxProperties")) {
    ...
    for (int i = 0; i < allGuidValueNameCount; i++) {
      RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid,
                                 allGuidValueNames[i], APOGUID_NOKEY);
    }
  } else {
    ...
    for (int i = 0; i < allGuidValueNameCount; i++) {
      ...
      if (RegistryHelper::valueExists(keyPath + L"\\FxProperties",
                                      allGuidValueNames[i])) {
        apoGuidString = RegistryHelper::readValue(keyPath + L"\\FxProperties",
                                                  allGuidValueNames[i]);
        valuenames.push_back(allGuidValueNames[i]);
      }

      RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid,
                                 allGuidValueNames[i], apoGuidString);
    }

    if (!valuenames.empty())
      RegistryHelper::saveToFile(
          keyPath + L"\\FxProperties", valuenames,
          L"backup_" + StringHelper::replaceIllegalCharacters(deviceName) +
              L"_" + StringHelper::replaceIllegalCharacters(connectionName) +
              L".reg");
  }
  ...
  RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid,
                             preMixChildGuidValueName, preMixValue);
  RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid,
                             postMixChildGuidValueName, postMixValue);

  RegistryHelper::writeValue(
      childApoPath L"\\" + deviceGuid, allowSilentBufferValueName,
      selectedInstallState.allowSilentBufferModification ? L"true" : L"false");
  if (selectedInstallState.autoAdjust) {
    if (RegistryHelper::valueExists(childApoPath L"\\" + deviceGuid,
                                    disableAutoAdjustValueName))
      RegistryHelper::deleteValue(childApoPath L"\\" + deviceGuid,
                                  disableAutoAdjustValueName);
  } else {
    RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid,
                               disableAutoAdjustValueName, L"true");
  }
  RegistryHelper::writeValue(childApoPath L"\\" + deviceGuid, versionValueName,
                             installVersion);
...
void DeviceAPOInfo::uninstall() {
  ...
  if (originalApoGuids[0] == APOGUID_NOKEY) {
    ...
    for (int i = 0; i < allGuidValueNameCount; i++) {
      if (originalApoGuids[i] == APOGUID_NOVALUE) {
        ...
        RegistryHelper::writeValue(keyPath + L"\\FxProperties",
                                   allGuidValueNames[i], originalApoGuids[i]);
      }
    }
  }

  if (RegistryHelper::keyExists(childApoPath) &&
      RegistryHelper::valueExists(childApoPath, deviceGuid))
  ...
  if (RegistryHelper::keyExists(childApoPath) &&
      RegistryHelper::keyEmpty(childApoPath))
...
wstring DeviceAPOInfo::getConnectionName() const { return connectionName; }

wstring DeviceAPOInfo::getDeviceName() const { return deviceName; }

wstring DeviceAPOInfo::getDeviceGuid() const { return deviceGuid; }

wstring DeviceAPOInfo::getDeviceString() const {
  return getConnectionName() + L" " + getDeviceName() + L" " + getDeviceGuid();
}

unsigned DeviceAPOInfo::getChannelCount() const { return channelCount; }

unsigned DeviceAPOInfo::getSampleRate() const { return sampleRate; }

unsigned long DeviceAPOInfo::getChannelMask() const { return channelMask; }

bool DeviceAPOInfo::isInput() const { return input; }

bool DeviceAPOInfo::isInstalled() const { return installed; }

bool DeviceAPOInfo::isEnhancementsDisabled() const {
  return enhancementsDisabled;
}

bool DeviceAPOInfo::isDefaultDevice() const { return defaultDevice; }

bool DeviceAPOInfo::isDisabled() const { return disabled; }

bool DeviceAPOInfo::isUnplugged() const { return unplugged; }
...
wstring DeviceAPOInfo::getPreMixChildGuid() { return preMixChildGuid; }

wstring DeviceAPOInfo::getPostMixChildGuid() { return postMixChildGuid; }
...
void DeviceAPOInfo::testAPOInstallation() {
  ...
  hr = enumerator->GetDevice(
      ((input ? L"{0.0.1.00000000}." : L"{0.0.0.00000000}.") + deviceGuid)
          .c_str(),
      &device);
  if (FAILED(hr))
    fail(L"GetDevice", hr);
  ...
  hr = audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 1000000 /*100 ms*/,
                               0, format, NULL);
  if (FAILED(hr))
...
void DeviceAPOInfo::fail(const wstring &functionName, HRESULT hr) {
  ...
  throw DeviceException(functionName + L" failed for device \"" + deviceName +
                        L"\" (" + msg + L")");
}
