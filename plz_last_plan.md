...
## Phase E (UX / Infra) — 사용자 피드백 반영

### E-1. 익명(비로그인) 모드 지원 및 데이터 마이그레이션
- **정책**: 앱 시작 시 로그인 없이 바로 사용 가능. (Guest 모드)
- **제한사항**: 오직 로컬 캐시에만 데이터 저장. AI 기능, API 호출, 클라우드 DB 연동은 비활성화.
- **마이그레이션**: 사용자가 가입/로그인 완료 시 로컬에 쌓인 Guest 데이터를 DB로 동기화.
- **충돌 처리**: 로컬 JSON과 DB 양쪽에 동일한 곡이 존재할 경우, 데이터 내 타임스탬프(`updated_at`)를 비교하여 **가장 최신 데이터**를 적용 (Last-Write-Wins 정책).

### E-2. 시스템 트레이 글씨 깨짐(Mojibake) 수정 (완료)
- `main.cpp`에서 `NOTIFYICONDATAA` 대신 유니코드 기반 `NOTIFYICONDATAW` 사용 적용.
- 트레이 메뉴("열기", "종료") 정상 한글 출력 완료. (추후 언어 설정에 따라 i18n 연동 예정)

### E-3. AI 모델 유지
- 당분간 `gemini-2.5-flash-lite` 로 변경하지 않고 기존 AI 모델 유지.
