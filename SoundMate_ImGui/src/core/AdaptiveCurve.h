...
// [1.5 -> 2.5] 1.5 는 청감 데이터 없이 잡은 보수적 첫 값이었고, 실제로
// 들어보니 EQ 효과가 너무 얌전했다. 실측(aespa) 기준 저역-중역 차가
// 2.44dB -> 3.93dB (1.6배) 가 된다.
// 상한이 아니라 눈금이다: 3.0 이면 에너지 +3.0dB 로, 먹먹했던 시절
// (+3.74dB, 리미터 개입률 94%) 에 근접하므로 여기서 더 올릴 때는
// limiterPct 로그를 반드시 확인해야 한다.
constexpr float kEnergyBudgetDb = 2.5f;
