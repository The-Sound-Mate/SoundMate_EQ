...
const char* RecordManager::SUPABASE_URL = "https://lpcarclwfgzlfczqflgo.supabase.co";
...
RecordManager::RecordManager() {
    ...
    m_recordDir  = std::string(localAppData) + "\\SoundMateEqualizer\\record";
...
void RecordManager::LoadCache() {
    if (!std::filesystem::exists(m_cacheFile)) {
        m_cache = {{"songs",{}},{"unsynced",json::array()},{"profile",{{"tendency","Balanced and clear sound"}}}};
        return;
    }
    try {
        std::ifstream f(m_cacheFile);
        m_cache = json::parse(f);
        if (!m_cache.contains("profile")) m_cache["profile"] = {{"tendency","Balanced and clear sound"}};
        if (!m_cache.contains("songs"))   m_cache["songs"]   = json::object();
        if (!m_cache.contains("unsynced"))m_cache["unsynced"]= json::array();
    } catch(...) {
        m_cache = {{"songs",{}},{"unsynced",json::array()},{"profile",{{"tendency","Balanced and clear sound"}}}};
    }
}
...
void RecordManager::LoadIntegratedHistory() {
    if (!std::filesystem::exists(m_historyFile)) return;
    try {
...
std::string RecordManager::NormalizeKey(const std::string& t, const std::string& a) {
    auto lower = [](std::string s){ for(auto&c:s)c=tolower(c); return s; };
    auto trim  = [](std::string s){
        auto p=s.find_first_not_of(" \t"); if(p!=std::string::npos)s=s.substr(p);
        auto q=s.find_last_not_of(" \t");  if(q!=std::string::npos)s=s.substr(0,q+1);
        return s;
    };
...
std::string RecordManager::GenerateTrackHash(const std::string& title, const std::string& artist) {
    ...
    if (CryptAcquireContextA(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
        if (CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hHash)) {
            if (CryptHashData(hHash, (const BYTE*)raw.c_str(), (DWORD)raw.size(), 0)) {
                BYTE hash[32];
                DWORD hashLen = 32;
...
std::string RecordManager::RefreshAccessToken(const std::string& refreshToken) {
    ...
    headers = curl_slist_append(headers, ("apikey: " + std::string(SUPABASE_KEY)).c_str());
    ...
    try {
        ...
        if (!newAt.empty()) {
            ...
            if (!newRt.empty()) tokenJson["refresh_token"] = newRt;
...
// Returns access_token (JWT), auto-refreshes if expired. Sets m_userId from sub claim.
std::string RecordManager::GetUserIdFromToken() {
    ...
    try {
        ...
        // Decode JWT payload
        auto p = at.find_first_of('.'); if (p==std::string::npos) return "";
        auto p2= at.find('.', p+1);    if (p2==std::string::npos) return "";
        ...
        long long now = (long long)std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
        if (exp > 0 && now >= exp - 60) { // refresh 1 min before expiry
            if (!rt.empty()) {
                std::string newAt = RefreshAccessToken(rt);
                if (!newAt.empty()) return newAt;
            }
...
std::pair<std::string,std::string> RecordManager::GetUserInfo() {
    if (!std::filesystem::exists(m_tokenFile)) return {"로그인되지 않음","N/A"};
    try {
        std::ifstream f(m_tokenFile);
        auto token = json::parse(f);
        std::string at = token.value("access_token","");
        if (at.empty()) return {"로그인되지 않음","N/A"};
        auto p=at.find('.'); auto p2=at.find('.',p+1);
        ...
        std::string name=meta.value("name",meta.value("full_name",meta.value("display_name",claims.value("email","사용자"))));
        std::string plan=meta.value("plan","무료 플랜 (Free)");
        if (m_userId.empty()) m_userId=claims.value("sub","");
        return {name,plan};
    } catch(...) { return {"로그인되지 않음","무료 플랜 (Free)"}; }
}

std::string RecordManager::GetUserPlanType() {
    auto [name,plan] = GetUserInfo();
    if (name=="로그인되지 않음"||name=="guest") return "free";
    std::string p=plan; for(auto&c:p)c=tolower(c);
    if (p.find("expert")!=std::string::npos) return "expert";
    if (p.find("pro")   !=std::string::npos) return "pro";
    if (p.find("beta")  !=std::string::npos) return "beta";
    return "free";
}

std::string RecordManager::GetUserTendency() {
    return m_cache.value("profile",json::object()).value("tendency","Balanced and clear sound");
}

void RecordManager::SaveUserTendency(const std::string& tendency, const json& prefsDict) {
    m_cache["profile"]["tendency"] = tendency;
    if (!prefsDict.empty()) m_cache["profile"]["prefs_dict"] = prefsDict;
    SaveCache();
}

void RecordManager::SaveSessionState(const std::vector<float>& gains, const std::vector<int>& bands,
    ...
    try {
        ...
        json state = {{"gains",gains},{"bands",bands},{"device",device},{"preset",preset},{"timestamp",ts.str()}};
...
SessionState RecordManager::LoadSessionState() {
    SessionState s;
    if (!std::filesystem::exists(m_sessionFile)) return s;
    try {
...
void RecordManager::SaveInteraction(const EQEntry& entry) {
    std::lock_guard<std::mutex> lk(m_mutex);
    if (entry.title.empty()) return;

    // AI/prompt 소스: 31밴드 모두 0이면 스킵
    if (entry.source=="AI"||entry.source=="prompt") {
        bool allZero = true;
        for (auto g : entry.gains31) if (std::abs(g)>0.01f){allZero=false;break;}
        if (allZero) return;
    }
    ...
    unsynced.erase(std::remove_if(unsynced.begin(), unsynced.end(), [&](const json& item){
    ...
    json eq_entry = {
        ...
        {"genre",   entry.genre},  {"source",   entry.source},
        {"eq_5",    entry.gains5}, {"eq_10",    entry.gains10},
        {"eq_15",   entry.gains15},{"eq_31",    entry.gains31},
        {"device_name", entry.deviceName}, {"prompt", entry.prompt}
    };
    unsynced.push_back(eq_entry);

    // 메모리 캐시 업데이트
    m_cache["songs"][key][entry.source] = {{"gains",entry.gains5},{"source",entry.source},
        {"multi_bands",{{"5",entry.gains5},{"10",entry.gains10},{"15",entry.gains15},{"31",entry.gains31}}}};
    SaveCache();
}

EQEntry* RecordManager::GetCachedEQ(const std::string& title, const std::string& artist) {
    std::string key = NormalizeKey(title, artist);
    static const std::vector<std::string> priority = {"direct","manual","prompt","AI"};
    ...
    auto tryGet = [&](const json& srcMap) -> EQEntry* {
        for (auto& src : priority) {
            if (srcMap.contains(src)) {
                ...
                if (mb.contains("5"))  e.gains5  = mb["5"].get<std::vector<float>>();
                if (mb.contains("10")) e.gains10 = mb["10"].get<std::vector<float>>();
                if (mb.contains("15")) e.gains15 = mb["15"].get<std::vector<float>>();
                if (mb.contains("31")) e.gains31 = mb["31"].get<std::vector<float>>();
                m_entryCache[key] = e;
    ...
    if (m_historyMap.count(key)) {
        json srcMap;
        for (auto& [s,rec] : m_historyMap[key]) srcMap[s] = rec.value("data",rec);
        if (auto* p = tryGet(srcMap)) return p;
    }
    if (m_cache["songs"].contains(key))
        if (auto* p = tryGet(m_cache["songs"][key])) return p;
    return nullptr;
}

bool RecordManager::ClearManualEQ(const std::string& title, const std::string& artist) {
    std::string key = NormalizeKey(title, artist);
    bool removed = false;
    std::vector<std::string> manualKeys = {"direct", "manual"};
    
    for (const auto& mKey : manualKeys) {
        if (m_cache["songs"].contains(key) && m_cache["songs"][key].contains(mKey)) {
            m_cache["songs"][key].erase(mKey); removed = true;
        }
        if (m_historyMap.count(key) && m_historyMap[key].count(mKey)) {
            m_historyMap[key].erase(mKey); removed = true;
        }
    }
...
bool RecordManager::ClearPromptEQ(const std::string& title, const std::string& artist) {
    std::string key = NormalizeKey(title, artist);
    bool removed = false;
    if (m_cache["songs"].contains(key) && m_cache["songs"][key].contains("prompt")) {
        m_cache["songs"][key].erase("prompt"); removed = true;
    }
    if (m_historyMap.count(key) && m_historyMap[key].count("prompt")) {
        m_historyMap[key].erase("prompt"); removed = true;
    }
    if (removed) SaveCache();
    return removed;
}
...
std::string RecordManager::SupabaseRequest(const std::string& method,
    ...
    headers = curl_slist_append(headers, ("Authorization: Bearer " + authTok).c_str());
    headers = curl_slist_append(headers, "Content-Type: application/json");
    if (method == "POST") {
        headers = curl_slist_append(headers, "Prefer: return=representation,resolution=merge-duplicates");
    } else {
...
std::vector<float> RecordManager::GetGlobalSongAverage(const std::string& title,
    ...
    try {
        ...
        std::string ep   = "/rest/v1/track_average?select=" + col + ",track!inner(track_hash)"
                           + "&track.track_hash=eq." + hash + "&limit=1";
...
std::vector<float> RecordManager::GetGlobalGenreAverage(const std::string& genre, int bandCount) {
    if (genre.empty()) return {};
    try {
        std::string col = "average_eq_" + std::to_string(bandCount);
        std::string ep  = "/rest/v1/genre_average?select=" + col + "&genre_name=eq." + genre + "&limit=1";
        auto resp = SupabaseRequest("GET", ep);
        if (resp.empty()) return {};
        auto arr = json::parse(resp);
        if (!arr.empty() && arr[0].contains(col) && !arr[0][col].is_null())
...
bool RecordManager::CheckUserHistory(const std::string& title, const std::string& artist) {
    if (m_userId.empty()) return false;
    try {
        std::string hash = GenerateTrackHash(title, artist);
        std::string ep   = "/rest/v1/track?select=id&track_hash=eq." + hash + "&limit=1";
        auto tresp = SupabaseRequest("GET", ep);
        auto tarr  = json::parse(tresp);
        if (tarr.empty()) return false;
        std::string trackId = tarr[0]["id"].get<std::string>();
        std::string ep2 = "/rest/v1/user_track_history?select=created_at&user_id=eq." + m_userId
...
bool RecordManager::SyncToDB() {
    ...
    if (accessToken.empty() || m_userId.empty()) return false;
    m_accessToken = accessToken;  // cache for other callers
    auto unsynced = m_cache["unsynced"];
    if (unsynced.empty()) return true;
    try {
        // 1. track 테이블 upsert
        json tracks = json::array();
        std::map<std::string,std::string> hashToId;
        for (auto& item : unsynced) {
            std::string h = GenerateTrackHash(item.value("title",""), item.value("artist",""));
            tracks.push_back({{"track_hash",h},{"title",item.value("title","")},
                              {"artist",item.value("artist","")},{"genre",item.value("genre","")}});
        }
        auto tresp = SupabaseRequest("POST", "/rest/v1/track?on_conflict=track_hash", tracks.dump(), accessToken);
        auto tarr  = json::parse(tresp);
        for (auto& r : tarr) hashToId[r["track_hash"].get<std::string>()] = r["id"].get<std::string>();
        ...
        for (auto& item : unsynced) {
            ...
            json hist = {{"user_id",m_userId},{"track_id",tid},
                         {"eq_5",item.value("eq_5",json())},{"eq_10",item.value("eq_10",json())},
                         {"eq_15",item.value("eq_15",json())},{"eq_31",item.value("eq_31",json())},
                         {"source",item.value("source","")},{"prompt",item.value("prompt","")},
        ...
        if (!upserts.empty())
            SupabaseRequest("POST","/rest/v1/user_track_history?on_conflict=user_id,track_id,source",upserts.dump(),accessToken);
        if (!inserts.empty())
            SupabaseRequest("POST","/rest/v1/user_track_history",inserts.dump(),accessToken);
...
bool RecordManager::ConsolidateLocalRecords(bool forceAll) {
    ...
    for (auto& p : fs::directory_iterator(m_recordDir)) {
        ...
        if (fn.rfind("applied_eq_",0)==0 && fn.size()>5 && fn.substr(fn.size()-5)==".json")
            files.push_back(p.path());
    }
    if (files.empty()) return false;
    if (!forceAll && files.size() < 2) return false;

    json history = json::array();
    if (fs::exists(m_historyFile)) {
        try { std::ifstream f(m_historyFile); history = json::parse(f); } catch(...) {}
    }
    ...
    for (auto& [k,v] : integrated) merged.push_back(v);
    try {
...
void RecordManager::LogAiError(const std::string& title, const std::string& artist,
