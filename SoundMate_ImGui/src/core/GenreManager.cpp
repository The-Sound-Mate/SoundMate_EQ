...
namespace {
    ...
    void LoadCacheIfNeeded() {
        ...
        try {
            ...
            if (!g_cache.is_object()) g_cache = json::object();
        } catch (...) {
            g_cache = json::object();
        }
    }
    ...
    bool LookupCache(const std::string& key, MusicInfo& outInfo, bool& outIsNegative) {
        std::lock_guard<std::mutex> lk(g_cacheMutex);
        LoadCacheIfNeeded();
        if (!g_cache.contains(key)) return false;
        auto& e = g_cache[key];
        int64_t ts = e.value("ts", (int64_t)0);
        bool valid = e.value("valid", false);
        int64_t ttl = valid ? kCacheTTLSuccess : kCacheTTLFailure;
        if (NowEpoch() - ts > ttl) return false;  // expired
